...
concommand.Add("ricelib_font_viewer", function(_, _, args)
    ...
    frame:SetSize(RiceLib.hudScale(1800, 900))
    frame:Center()
    frame:MakePopup()
    frame:SetTitle("RiceLib Font Viewer " .. args[1])
    frame:SetTheme("ModernDark")

    local panel, bar, barGrip = RiceLib.VGUI.ScrollPanel(frame)
    panel:Dock(FILL)
    panel:SetTheme("ModernDark")

    RiceLib.VGUI.RegisterFontAdv(args[1], "FontView" .. args[1])

    for i = 1, 20 do
        local button = vgui.Create("DLabel", panel)
        button:Dock(TOP)
        button:DockMargin(0, RiceLib.hudScaleY(5), 0, 0)
        button:SetText(" " .. tostring(i * 5) .. " " .. test_str)
        button:SetTheme("ModernDark")
        button:SetTall(RiceLib.hudScaleY(i * 5 + 10))
...
concommand.Add("ricelib_vgui_theme_viewer", function()
    local frame = vgui.Create("DFrame")
    frame:SetSize(RiceLib.hudScale(500, 500))
    frame:Center()
    frame:MakePopup()
    frame:SetTitle("RiceLib VGUI Theme Viewer")
    frame:SetTheme("ModernDark")

    local panel = RiceLib.VGUI.ScrollPanel(frame)
    panel:Dock(FILL)
    panel:SetTheme("ModernDark")

    for k in SortedPairs(RiceLib.VGUI.Theme) do
        local button = vgui.Create("DButton", panel)
        button:Dock(TOP)
        button:DockMargin(0, RiceLib.hudScaleY(5), 0, 0)
        button:SetText(k)
        button:SetTheme(k)
        button:SetTall(RiceLib.hudScaleY(50))
...
concommand.Add("ricelib_vgui_colortheme_viewer", function()
    ...
    for k in SortedPairs(RiceLib.VGUI.ColorTheme) do
        local button = vgui.Create("DButton", panel)
        button:Dock(TOP)
        button:DockMargin(0, RiceLib.hudScaleY(5), 0, 0)
        button:SetText(k)
